...
from agents.state_management.output_manner_model import ExtractorOutput
...
class MannerExtractNode(BaseNode):
  ...
  def execute(self, state: MannerState, config: RunnableConfig):
    ...
    # TODO: .env переменные
    last_n_human_messages = [m for m in messages if isinstance(m, HumanMessage)][-n_messages:]  # Получаем все сообщения ПОЛЬЗОВАТЕЛЯ
    ...
    if result["is_to_memory"]:
      return Command(
        update={
          ...
          "remaining_steps_to_check_manner": steps_to_check,  # TODO: в .env вынести
