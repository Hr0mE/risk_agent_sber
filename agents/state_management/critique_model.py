...
class CritiqueDecisionModel(BaseModel):
    ...
    final_decision: str = Field(description="Итоговое решение, должно быть одно из следующих: finalize (если нет новой критики и речь можно считать написаной), search (требуется поиск данных в интернете), rag (если требуется поиск по внутренней базе документов)")
