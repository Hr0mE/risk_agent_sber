...
class CritiqueDecisionModel(BaseModel):
    ...
    final_decision: str = Field(description="Итоговое решение, должно быть одно из следующих: finalize (если нет новой критики и речь можно считать написаной), rag (если требуется поиск по внутренней базе документов)") # , search (требуется поиск данных в интернете)
