...
NODE_METADATA: Dict[NodeNames, NodeMeta] = {
    NodeNames.EXTRACT_MANNER: NodeMeta(
        label="Извлечение манеры", description="Извлечение манеры пользователя из запроса", emoji="👩‍🔬"
    ),
    NodeNames.WRITE_MANNER: NodeMeta(
        label="Запись манеры", description="Запись полученной манеры пользователя в память", emoji="📝"
    ),
    NodeNames.GET_MANNER: NodeMeta(
        label="Получение манеры", description="Извлечение манеры пользователя из памяти", emoji="💭"
    ),
    NodeNames.STORE_QUESTION: NodeMeta(
        label="Сбор вопросов", description="Сбор вопросов во временный буфер", emoji="❓"
    ),
    ...
    NodeNames.GET_FAQ: NodeMeta(
        label="Извлчение вопросов", description="Извлечение часто задаваемых вопросов", emoji="💭"
    ),
    NodeNames.REASON: NodeMeta(
        label="Анализ и рассуждение", description="Генерация мыслей и анализ вопроса пользователя", emoji="🧠"
    ),
    ...
    NodeNames.SEARCH: NodeMeta(label="Веб-поиск", description="Поиск информации в интернете", emoji="🔍"),
    NodeNames.RAG: NodeMeta(
        label="Поиск по базе знаний", description="Поиск во внутренних документах и базах данных", emoji="📚"
    ),
    NodeNames.WRITE: NodeMeta(
        label="Генерация ответа", description="Создание черновика ответа пользователю", emoji="✍️"
    ),
    NodeNames.FINALIZE: NodeMeta(
        label="Финальный ответ", description="Подготовка и выдача окончательного ответа", emoji="✅"
    ),
    NodeNames.CRITIQUE: NodeMeta(
        label="Критический анализ", description="Проверка и улучшение сгенерированного ответа", emoji="🔎"
    ),
